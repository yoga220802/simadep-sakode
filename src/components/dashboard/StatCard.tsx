...
export default function StatCard({ data }: StatCardProps) {
	...
	return (
		<div className='rounded-xl border-2 border-[var(--simadep-border)] bg-white p-6 transition-shadow hover:shadow-lg'>
			<div className='flex items-start justify-between'>
				<div className='flex items-center gap-4'>
					...
					<p className='text-xl font-semibold text-[var(--simadep-muted)]'>{data.title}</p>
				</div>
			</div>

			<div className='mt-5 flex items-end justify-between'>
				<p className='text-5xl font-bold text-[var(--color-text-main)]'>{data.value}</p>
				{hasChange && (
					<span
						className={`flex items-center gap-1 text-base font-semibold ${
							isIncrease ? "text-[var(--color-accent)]" : "text-[var(--color-secondary)]"
						}`}>
